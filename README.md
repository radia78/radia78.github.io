...
Collaborated with RoundtableML to create an open-source implementation of Google's text-to-image Muse model. Differed from most implementation by using multi-axis attention on the large resolution model, reducing computation time from $O(n^{2})$ to $O(n \cdot log(n))$.
